...
broadcast-style momentum curve. That problem was solved upstream, for football, and it is genuinely
sport-agnostic — exponential decay and Gaussian smoothing don't care what game produced the
impulses.

The second is *getting the event stream*. That's the actual hard part. Typing one out during a
match is slow and you get it wrong, and in rugby the thing you most need — *where* on the pitch it
happened — is exactly what a keyboard vocabulary can't express. `tracer/` exists for this, and it's
where roughly 80% of the code lives.
...
clock — tennis, say — would map points and games onto a synthetic axis and the engine wouldn't
notice.
...
The two implementations are genuinely different in kind, which is the interesting part:

- **Football** keys off discrete threat moments — shot, big chance, goal, sustained pressure — so
  its weights are a flat JSON lookup table.
...
That asymmetry is the proof the abstraction was worth having. A design where weight is always a
table lookup would have forced rugby into a shape that doesn't fit it.
...
alternative — one class per (sport, provider) pair — multiplies: three sports and four providers
is twelve classes, and eleven of them are copy-paste. Here it's seven.
...
pins `origin_factor` to 1.0. It ports the same decay maths from `core/engine.py`, but it is not
reproducing the authoritative curve and shouldn't be read as if it were.
...
That's what makes `validate.py` possible in its current form — it can literally call
`RugbySport().translate()` and `MomentumEngine.compute()` rather than reimplementing a schema check
that would drift. The cost is that the tracer can't be a thin client, and browser latency shows up
as timestamp jitter (see page 3).

**No live chart.** Chart generation stays a manual `momentum.py` step. Rendering during capture
would mean matplotlib in the request path for no benefit — you can't act on a momentum curve
mid-match anyway.
...
— and minute is the chart's x-axis. This is the single largest known inaccuracy in the whole system
and it is not fixable without a different input model.
