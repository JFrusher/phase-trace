...
Making the analyst type that in would be one more thing to get wrong under time pressure. Most of
it is already sitting in the traced line — you just have to read it.
...
*team* is offered as a correction, and with two teams a wrong guess is always one click from right.
...
The value of naming these three categories is that it tells you what to do when the tool is wrong.
Green wrong means the geometry is wrong — that's a bug or a bad trace. Orange wrong means you
didn't tell it, which is one click.
...
- **Carried out?** The mark is simply where the carrier crossed, and the kick-to-touch law has
  nothing to say about it.
- **Kicked out, with a penalty armed as `kick_to_touch`?** The throw stays with the **kicking**
  team, and the ground is kept. This is why the armed case is tested *before* the law below and
  bypasses it — a penalty to touch is not an open-play kick.
...
That last default is a deliberate choice under uncertainty. A top-down trace cannot see a bounce,
so the tool takes the stricter reading and offers the alternative: `ChainOrigin` carries an
`alt_mark`, and the `⇄` button on the chip swaps between them. Both marks are lawful; only the
analyst knows which happened.

One quirk documented in `geometry.py` and worth knowing: for kicks, "out of touch" is tested by
*proximity* (`ends_in_touch`) rather than *crossing*. The browser stops reporting `mousemove` once
the cursor leaves the image, so a ball kicked out is a path that **stops** at the edge, never one
that visibly crosses it. Carries use the strict crossing test instead, because a winger runs inside
that margin all game.
...
in-goal is pure geometry (`in_goal_defender`) — it never depends on who happened to be carrying,
which is the point, since both teams end up in both in-goals over a match.
...
The receiving side, with **no mark** — the ball is in open play, so the next press is free-hand
rather than snapped.
...
The fall-through, marked at where the trace ended. Possession is assumed lost at the breakdown —
the coarse knock-on / jackal / held-in-the-tackle case. There's a `ponytail:` comment in the source
acknowledging this is an assumption: tap `Z` or `X` if possession was actually retained, as when a
penalty is played on.

## What the taps do

`S` (scrum) and `F` (penalty) both end the play and award **against whoever was carrying**. That's
right most of the time and wrong occasionally, and the chip's team badge fixes it in one click.

An `F` tap does three extra things:

1. Logs a `penalty_won` event carrying `conceded_by` — the side that gave it away — so a coach can
   map their own discipline, not just penalties won.
2. Arms the option chooser: *to touch · at goal · tap · scrum*, defaulting to `kick_to_touch`.
3. Sets `armed_next_action`, which forces the next stroke to be a KICK. To-touch and at-goal are
   both kicks by definition; letting the recognizer read a short-drawn penalty kick as a carry would
   put the ball in the wrong hands.

There's also a second chooser on a penalty chip for *why* it was given (offside · high · ruck ·
scrum · foul · other). It writes onto the `penalty_won` event as an optional extra the momentum
translator ignores. Absent until picked, which is the general rule for annotations here.
...
drawn through the posts is taken as a successful kick — the analyst draws it that way on purpose.
A kick that falls short or crosses wide is a miss.
...
— defenders field those far more often than the chase wins them.
...
docstring says so plainly. Set pieces get no weight of their own — a lineout is not itself threat
creation, it's context for the possession it starts, and crediting it separately would double-count
that possession. Cards are handled the same way and for the same reason: going to 14 men isn't
threat creation, and the pressure it invites is already counted by the phases that follow.

The football side of this repo was validated against published Flashscore graphics. The rugby side
has not been validated against anything, because nobody has traced a real match end to end yet.
That's not a gap in the documentation; it's a gap in the data, and it's stated in the code, the
README, and here.
