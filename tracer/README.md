...
2. Tap **`A` or `Space`** when the play dies (tackle/ruck/whistle). The tap is
   the authoritative end, not releasing the button; release is a defensive
   fallback. Tracing the ball **out of play** (over a touchline or a dead-ball
   line) ends the chain by itself, because in law it is over: keep drawing
   straight out and the lineout or drop-out lands without a tap.
...
A collapsible legend pane sits right of the pitch listing both vocabularies,
line colours and every key. It is generated from `config.py` and `canvas.py`,
so it cannot drift from what the app dispatches (`tests/test_legend.py`
enforces it).
...
A scrum and a penalty are tapped, because a line cannot show either.
...
segment is a KICK however short it was drawn. A restart read as a carry would
leave the ball with the side that just conceded. The restart chip sits below
the spot rather than on it, so it can't swallow the press that starts the kick.

The result appears as a **chip** on the pitch, above its mark, and the chip is
the correction UI:

- **Click the team name** to swap it.
...
- The **type is never editable where geometry settles it**: a lineout does not
  become a scrum. The two exceptions are the things a line genuinely cannot
  show, and both get a chooser row instead:
...
"why did my trace disappear" case). Each report holds path stats (points,
duration, Hz, net px), the boundary baselines and every candidate's evidence
score including the top near-miss rejects (what a missed turn actually
scored), a per-segment score table (each feature's value, weight, and
contribution per class, final probabilities, confidence margin,
`CARRY->KICK (hint)` when a tap relabelled it), and every tap-correlation
decision. The canvas draws a white dot at each picked boundary.

- **Replay** injects any `tracer/fixtures.py` scenario (or a saved trace)
  through the real pipeline, giving the same canvas redraw and the same
  report. Start the clock first or minute stamps read ~0.
- **Save last trace** snapshots the last chain's raw inputs to
  `tracer/dev_traces/` (gitignored). Replaying the file reproduces the
...
The loop: save the misbehaving trace in dev mode, promote it with its expected
truth, then run the tools. `python -m tracer.sweep` grid-sweeps any constants
over the whole corpus (39 synthetic scenarios plus promoted real traces).
`python -m tracer.fit` proposes classification weights by softmax regression
over the same corpus. `python -m tracer.calibrate` is `fit` plus the segment
corrections you made live in the app, which are logged as you make them; run
it pre-game. None of the three writes `config.py`. They print a proposal and
you make the edit.

## Manual test recipe (browser half; the pure logic is pytest-covered)

1. `python -m tracer.app`, start a match, start the clock.
2. **Capture spike check**: hold the mouse down and drag while tapping letters
   and holding Shift. The white line must follow the cursor and no key may
   interrupt the drag. If taps drop or the drag aborts, file it before
   trusting anything else — the whole input model rests on this holding.
...
Nobody has traced live footage end to end yet, so the numbers most likely to
move are the boundary accept level, the ~27m geometric kick threshold, and the
`origin_factor` weights in `translators/rugby_weights.json`. Those weights are
judgement calls with no reference data behind them.

Both ways into the tuning loop above work for closing that gap. [TUNING.md](TUNING.md)
covers the self-correction path in full.
...
- The match clock is a stopwatch you start and pause to stay roughly with
  ...
  input model, so **event minutes are approximate**, and minute is the chart's
  x-axis.
